...
pipe[0] = {
    x : cvs.clientWidth, // Первый объект будет находится за экраном. это дополнительный не тот который первоначально отрисовался.
    y:0
}
...
var grav=1; // Переменная гравитация чтобы птичка все время падала.
...
function draw() {//Эта функция для отрисовки картинок на холсте
    ...
    for (var i=0; i<pipe.length; i++){
        ctx.drawImage(pipeUp, 100, 0); // 100 это позиция по x и 0 это позиция по y
        ctx.drawImage(pipeBottom, 100, 0 + pipeUp.height + gap); //Здесь чтобы опустить трубу мы указываем что возьми высоту трубы pipeUp и добавь отступ из переменной gap  
    }
